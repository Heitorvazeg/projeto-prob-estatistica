...
vars_quali <- c("Regiao", "Sexo", "Grau_instrucao", "Faixa_Anos_Estudo", "Faixa_Horas_Trabalho")
...
# --- 3. Gerar Gráfico Agrupado Melhorado ---
# Extrair variáveis e pesos
df_vars <- dados_pnad_survey$variables
df_vars$pesos <- weights(dados_pnad_survey)

# Transformar para formato longo e calcular porcentagens
df_plot <- df_vars %>%
  ...
  summarise(Total = sum(pesos), .groups = 'drop') %>%
  group_by(Variavel) %>%
  mutate(Prop = Total / sum(Total)) %>%
  ungroup()

# Melhorar os nomes para o gráfico (opcional, deixa mais bonito)
# Exemplo: Trocar "_" por espaço
df_plot$Variavel <- gsub("_", " ", df_plot$Variavel)
...
ggplot(df_plot, aes(x = Prop, y = reorder_within(Categoria, Prop, Variavel))) +
  ...
  scale_x_continuous(labels = scales::percent_format(), expand = expansion(mult = c(0, 0.2))) +
  scale_y_reordered() + # Função mágica para ordenar facets individualmente
  labs(title = "Perfil Demográfico e Socioeconômico",
       subtitle = "Distribuição percentual da população estimada",
       x = NULL, y = NULL) +
  theme_minimal(base_size = 12) +
  theme(
    panel.grid.major.y = element_blank(), # Limpa linhas desnecessárias
