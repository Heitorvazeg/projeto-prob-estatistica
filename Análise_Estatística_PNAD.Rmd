...
```{r renda_ocupacao_regiao_corrigido, warning=FALSE, message=FALSE, fig.width=10, fig.height=15}
...
# (Seus dados df_renda_ocup já estão criados)

ggplot(df_renda_ocup, aes(x = Renda_Media, y = reorder(Cat_ocup, Renda_Media))) +
  
  # 1. Use uma cor única. O preenchimento por categoria aqui é só ruído visual.
  geom_col(alpha = 0.85, fill = "#0072B2") + 
  
  # 2. Adiciona os rótulos de Renda Média nas barras para clareza
  geom_text(aes(label = scales::comma(Renda_Media, accuracy = 1)), 
            hjust = -0.1,  # Posição um pouco fora da barra
            size = 3.0) +
  
  # 3. A GRANDE MUDANÇA: 1 coluna, N linhas. E scales = "free" (libera X e Y)
  facet_wrap(~ Regiao, ncol = 1, scales = "free") + 
  
  # 4. Ajusta a expansão do eixo X para caber os rótulos de texto
  ...
  labs(
    ...
    x = "Renda Média (R$)",
    y = NULL # O eixo Y é autoexplicativo, removemos o título
  ) +
  theme_minimal(base_size = 11) +
  theme(
    strip.text = element_text(face = "bold", size = 12), # Título da Região
    ...
    legend.position = "none" # Removemos a legenda (desnecessária)
  )
